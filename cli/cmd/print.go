...
func printOut(i interface{}) {
	...
	case []cwctl.Computer:
		tbl = table.New("ID", "COMPUTER", "CLIENTID", "CLIENT", "IP", "OS", "AGENT", "STATUS", "LastContact", "LastUser")
		for _, v := range i {
			tbl.AddRow(v.Id, v.ComputerName, v.Client.Id, v.Client.Name, v.LocalIPAddress, v.OperatingSystemName, v.RemoteAgentVersion, v.Status, v.RemoteAgentLastContact, v.LastUserName)
		}
	case []cwctl.Command:
