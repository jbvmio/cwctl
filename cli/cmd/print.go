...
func printOut(i interface{}) {
	...
	case []cwctl.Computer:
		tbl = table.New("ID", "COMPUTER", "CLIENT", "IP", "OS", "AGENT", "STATUS", "LastContact", "LastUser")
		for _, v := range i {
			tbl.AddRow(v.Id, v.ComputerName, fmt.Sprintf("%s [ %s ]", v.Client.Name, v.Client.Id), v.LocalIPAddress, v.OperatingSystemName, v.RemoteAgentVersion, v.Status, v.RemoteAgentLastContact, v.LastUserName)
		}
	case []cwctl.Command:
